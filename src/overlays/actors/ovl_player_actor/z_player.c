...
/* DC-only custom DL op: toggle the renderer's damage-flash fog-color redirect.
   The PVR fog-color register is global/once-per-frame, so Link's red i-frame fog
   (Gfx_SetFog2 below) can't override the scene fog color and would flash gray.
   Emitted as a bracket pair around Link's OPA draw; the interpreter bakes the red
   into the vertex color instead. See sDamageFlash in src/dreamcast/main.c. */
#define gDPDamageFlashToggle(pkt)    \
    {                                \
        Gfx* _g = (Gfx*)(pkt);       \
        _g->words.w0 = 0x424C4E44;   \
        _g->words.w1 = 0x44464C53;   \
    }                                \
    (void)0
...
void Player_Draw(Actor* thisx, PlayState* play2) {
    ...
    if (!(this->stateFlags2 & PLAYER_STATE2_29)) {
        ...
        if (this->invincibilityTimer > 0) {
            gDPDamageFlashToggle(POLY_OPA_DISP++);
