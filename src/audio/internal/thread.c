...
AudioTask* AudioThread_UpdateImpl(void) {
...
#ifdef LINUX
    {
        static int audioUpdDbg = 0;
        if (audioUpdDbg < 10) {
            printf("AudioThread_Update[%d]: resetStatus=%d writePos=%d readPos=%d\n",
                   (int)gAudioCtx.totalTaskCount, (int)gAudioCtx.resetStatus,
                   (int)gAudioCtx.threadCmdWritePos, (int)gAudioCtx.threadCmdReadPos);
            audioUpdDbg++;
        }
    }
#endif
...
#ifdef LINUX
    /* Use DC ring buffer fill level as feedback, like N64's AI hardware register.
     * This prevents overproduction and keeps latency low. */
    samplesRemainingInAi = 512;//(u32)(audio_dc_get_buffered_bytes() / (int)sizeof(s16));
#else
    samplesRemainingInAi = osAiGetLength() / 4;
#endif

#ifndef LINUX
    /* On DC, osAiSetNextBuffer writes to non-existent N64 AI registers.
     * DC audio output is handled directly via audio_dc_play(). */
...
#ifdef __DREAMCAST__
    curAiBufferL = gAudioCtx.aiBuffersL[index];
    curAiBufferR = gAudioCtx.aiBuffersR[index];
#else
    curAiBuffer = gAudioCtx.aiBuffers[index];
#endif

#ifdef __DREAMCAST__
    gAudioCtx.aiBufLengths[index] = 448;
#else
    ...
    if (gAudioCtx.resetStatus == 0) {
        ...
        while (osRecvMesg(gAudioCtx.threadCmdProcQueueP, (OSMesg*)&sp4C, OS_MESG_NOBLOCK) != -1) {
...
#ifdef LINUX
            {
                static int cmdDbg = 0;
                if (cmdDbg < 10) {
                    printf("  ProcessCmds: msg=0x%08x (rd=%d wr=%d)\n",
                           (unsigned)sp4C, (unsigned)(sp4C >> 8) & 0xFF, (unsigned)sp4C & 0xFF);
                    cmdDbg++;
                }
            }
#endif
    ...
    gAudioCtx.curAbiCmdBuf =
        AudioSynth_Update(gAudioCtx.curAbiCmdBuf, &abiCmdCnt, curAiBufferL, curAiBufferR,
            gAudioCtx.aiBufLengths[index]);
    ...
    gAudioCtx.audioRandom = gAudioCtx.audioRandom + gAudioCtx.aiBuffersL[index][gAudioCtx.totalTaskCount & 0xFF];
#else
    gAudioCtx.curAbiCmdBuf =
        AudioSynth_Update(gAudioCtx.curAbiCmdBuf, &abiCmdCnt, curAiBuffer,
            gAudioCtx.aiBufLengths[index]);
...
#ifdef __DREAMCAST__
    /*
     * AICA hardware mixing: AudioSynth_Update drove the AICA voices directly
     * (AicaSynth_Update). There is no software-mixed output buffer to stream, so
     * the old audio_dc_play()/snd_stream path is retired. (curAiBufferL/R remain
     * allocated but unused; the stream is never started, leaving all 64 AICA
     * channels available to the voice driver.)
     */
    (void)curAiBufferL;
    (void)curAiBufferR;

    /* Skip RSP task packaging on DC - no RSP hardware */
...
#ifdef LINUX
    {
        static int globalCmdDbg = 0;
        if (globalCmdDbg < 15) {
            printf("  GlobalCmd: op=0x%02x arg0=%d arg1=%d arg2=%d data=0x%08x\n",
                   (unsigned)cmd->op, (int)cmd->arg0, (int)cmd->arg1, (int)cmd->arg2, (unsigned)cmd->asUInt);
            globalCmdDbg++;
        }
    }
#endif
...
#ifdef LINUX
    {
        static int schedDbg = 0;
        if (schedDbg < 10) {
            printf("ScheduleProcessCmds: readPos=%d writePos=%d\n",
                   (int)gAudioCtx.threadCmdReadPos, (int)gAudioCtx.threadCmdWritePos);
            schedDbg++;
        }
    }
#endif
