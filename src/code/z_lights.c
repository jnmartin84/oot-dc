...
void Lights_Draw(Lights* lights, GraphicsContext* gfxCtx) {
    ...
    //if ((u32)lights->numLights > max_lights) { max_lights = (u32)lights->numLights;
    //printf("current lights %d max lights %d\n", (u32)lights->numLights, max_lights);
    //}
//    if (max_lights > 5) {
  //      printf("exceeded xmtrx light limit, exiting.\n");
    //    exit(-1);
    //}
    light = &lights->l.l[0];
    i = 0;
...
#ifdef __DREAMCAST__
                // Occlusion is handled in Lights_DrawGlow on DC (this callback path doesn't run); always pass here.
...
#ifdef __DREAMCAST__
            {
                // No readable Z-buffer on PVR; raycast eye->glow against collision for occlusion.
                Vec3f glowPos;
                Vec3f glowHitPos;
                CollisionPoly* glowHitPoly;
                s32 glowHitBgId;

                glowPos.x = params->x;
                glowPos.y = params->y;
                glowPos.z = params->z;
                params->drawGlow = !BgCheck_AnyLineTest3(&play->colCtx, &play->view.eye, &glowPos, &glowHitPos,
                                                         &glowHitPoly, 1, 1, 1, 1, &glowHitBgId);
            }
#endif
            if (params->drawGlow) {
