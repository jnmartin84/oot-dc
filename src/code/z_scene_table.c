...
#ifdef __DREAMCAST__
    sp83 = (s8)(shz_sincosu16(play->gameplayFrames * 1500).cos * 127.0f);
#else
...
#ifdef __DREAMCAST__
    sp7B = (s8)(shz_sincosu16((play->gameplayFrames * 1500) & 0xFFFF).cos * 127.0f);
#else
...
Gfx* Gfx_TwoTexScrollPrimColor(GraphicsContext* gfxCtx, s32 tile1, u32 x1, u32 y1, s32 width1, s32 height1, s32 tile2,
    ...
    gDPSetTileSize(displayList + 1-1, tile1, x1, y1, x1 + ((width1 - 1) << 2), y1 + ((height1 - 1) << 2));
    gDPTileSync(displayList + 2);
    gDPSetTileSize(displayList + 3-2, tile2, x2, y2, x2 + ((width2 - 1) << 2), y2 + ((height2 - 1) << 2));
    gDPSetPrimColor(displayList + 4-2, 0, 0, r, g, b, a);
    gSPEndDisplayList(displayList + 5-2);
...
void Scene_DrawConfigDeathMountainCrater(PlayState* play) {
#ifdef __DREAMCAST__
    s8 sp6F = (s8)(shz_sincosu16((play->gameplayFrames * 1500) & 0xFFFF).cos * 127.0f);
    s8 sp6E = sp6F;
#else
