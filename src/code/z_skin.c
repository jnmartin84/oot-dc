...
const shz_vec4_t zerovec = (shz_vec4_t) { .x = 0, .y = 0, .z = 0, .w = 0};
shz_vec4_t wvec = (shz_vec4_t) { .x = 0, .y = 0, .z = 0, .w = 0};

MtxF gSkinLimbMatrices[60]; // holds matrices for each limb of the skeleton currently being drawn
extern void SkinMatrix_Vec3fMtxFMultXYZ_NoLoad(Vec3f* src, Vec3f* dest);
void Skin_UpdateVertices(MtxF* mtx, SkinVertex* skinVertices, SkinLimbModif* modifEntry, Vtx* vtxBuf, Vec3f* pos) {
    ...
    shz_xmtrx_load_transpose_4x4((const shz_mat4x4_t*)mtx);
    shz_xmtrx_write_col(3, zerovec);
    ...
    for (vertexEntry = skinVertices; vertexEntry < &skinVertices[modifEntry->vtxCount]; vertexEntry++) {
        ...
        normal = shz_xmtrx_transform_vec3(sp64);

//        SkinMatrix_Vec3fMtxFMultXYZ_NoLoad(/* mtx,  */&sp64, &normal);
