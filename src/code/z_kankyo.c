...
Vtx gFakeSunVtx[] = {
    VTX(   -31,    -31,     0,     0x0,     0x0, 0xFF, 0xFF, 0xFF, 0xFF), // 0
    VTX(    32,    -31,     0, 0x7E0/2,     0x0, 0xFF, 0xFF, 0xFF, 0xFF), // 1
    VTX(   -31,     32,     0,     0x0, 0x7E0/2, 0xFF, 0xFF, 0xFF, 0xFF), // 2
    VTX(    32,     32,     0, 0x7E0/2, 0x7E0/2, 0xFF, 0xFF, 0xFF, 0xFF), // 3
};
...
// Sun/moon sit only ~3000 units from the eye, so distant terrain (farther than that) would draw behind
// them. Push each disc K times farther from the eye and scale it up by K: identical on-screen size, but
// it now sorts behind all real geometry while still drawing in front of the sky. Raise to push farther
// (until it clips past the far plane / hides behind the sky); lower if near geometry should still occlude.
#define SUNMOON_DEPTH_PUSH 3.0f
...
void Environment_DrawSunAndMoon(PlayState* play) {
    ...
    if (gSaveContext.save.entranceIndex != ENTR_HYRULE_FIELD_0 || ((void)0, gSaveContext.sceneLayer) != 5) {
        Matrix_Translate(play->view.eye.x + play->envCtx.sunPos.x * SUNMOON_DEPTH_PUSH,
                         play->view.eye.y + play->envCtx.sunPos.y * SUNMOON_DEPTH_PUSH,
                         play->view.eye.z + play->envCtx.sunPos.z * SUNMOON_DEPTH_PUSH, MTXMODE_NEW);
        ...
        noFlare = 0;
        Matrix_Scale(scale * SUNMOON_DEPTH_PUSH, scale * SUNMOON_DEPTH_PUSH, scale * SUNMOON_DEPTH_PUSH, MTXMODE_APPLY);
        ...
        gSPDisplayList(POLY_OPA_DISP++, gFakeSunDL);//gSunDL);
...
void Environment_DrawLensFlare(PlayState* play, EnvironmentContext* envCtx, View* view, GraphicsContext* gfxCtx,
    ...
    if (cosAngle < 0.0f) {
        ...
        if (isSun) {
            ...
            // PVR has no readable Z-buffer; test sun occlusion by raycasting eye->sun against collision.
            // Raycast to the VISUAL sun (eye + sunPos*DEPTH_PUSH); the un-pushed `pos` sat only ~1/3 of
            // the way out, so occluders past ~sunPos never got hit and the sun wouldn't fade behind them.
            sunRayEnd.x = view->eye.x + (pos.x - view->eye.x) * SUNMOON_DEPTH_PUSH;
            sunRayEnd.y = view->eye.y + (pos.y - view->eye.y) * SUNMOON_DEPTH_PUSH;
            sunRayEnd.z = view->eye.z + (pos.z - view->eye.z) * SUNMOON_DEPTH_PUSH;
            if (BgCheck_AnyLineTest3(&play->colCtx, &view->eye, &sunRayEnd, &sunHitPos, &sunHitPoly, 1, 1, 1, 1,
                                     &sunHitBgId) ||
                screenPos.x < 0.0f || screenPos.y < 0.0f || screenPos.x > SCREEN_WIDTH ||
        ...
        for (i = 0; i < ARRAY_COUNT(lensFlareTypes); i++) {
            ...
            u8 alphaToUse = alpha * envCtx->lensFlareAlphaScale;
            if (alphaToUse > 224) alphaToUse = 224;
            if (alphaToUse < 0) alphaToUse = 0;

            POLY_XLU_DISP = func_800947AC(POLY_XLU_DISP++);
            gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, lensFlareColors[i].r, lensFlareColors[i].g, lensFlareColors[i].b,
                            alphaToUse);//alpha * envCtx->lensFlareAlphaScale);
