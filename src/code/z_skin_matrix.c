...
#ifdef __DREAMCAST__
    shz_vec4_t out = shz_mat4x4_transform_vec4((const shz_mat4x4_t*)mf, shz_vec3_vec4(shz_vec3_from(*src), 1.0f));
    *xyzDest = shz_vec3_to(Vec3f, out.xyz);
    *wDest = out.w;
#else
 ...
 * \f]
 */
void SkinMatrix_Vec3fMtxFMultXYZ_NoLoad(Vec3f* src, Vec3f* dest) {
    *dest = shz_vec3_to(Vec3f, shz_xmtrx_transform_vec3(shz_vec3_from(*src)));
}
...
#ifdef __DREAMCAST__
    shz_vec4_t srcv = (shz_vec4_t) { .x = src->x , .y = src->y, .z = src->z, .w = 1};
    shz_vec4_t dstv;
    dstv = shz_mat4x4_transform_vec4((const shz_mat4x4_t*)mf, srcv);
    dest->x = dstv.x;
    dest->y = dstv.y;
    dest->z = dstv.z;
    //shz_vec3_from(*src)));
...
#ifdef __DREAMCAST__
    shz_xmtrx_load_apply_store_4x4((shz_mat4x4_t*)dest, (const shz_mat4x4_t*)mfA, (const shz_mat4x4_t*)mfB);
#else
    f32 cx;
    f32 cy;
    f32 cz;
...
#if 0 //def __DREAMCAST__
    shz_mat4x4_init_identity((shz_mat4x4_t*)mf);
#else
    mf->xx = 1.0f;
    mf->yx = 0.0f;
    mf->zx = 0.0f;
...
#ifdef __DREAMCAST__
    shz_mat4x4_copy((shz_mat4x4_t*)dest, (const shz_mat4x4_t*)src);
#else
    dest->xx = src->xx;
    dest->yx = src->yx;
    dest->zx = src->zx;
...
#ifdef __DREAMCAST__
    shz_mat4x4_init_scale((shz_mat4x4_t*)mf, x, y, z);
#else
    mf->yx = 0.0f;
    mf->zx = 0.0f;
    mf->wx = 0.0f;
...
void SkinMatrix_SetRotateZYX(MtxF* mf, s16 x, s16 y, s16 z) {
#ifdef __DREAMCAST__
    shz_mat4x4_init_rotation_zyx((shz_mat4x4_t*)mf,
                                 SHZ_S16_ANGLE(z),
                                 SHZ_S16_ANGLE(y),
                                 SHZ_S16_ANGLE(x));
#else
    f32 cos;
#ifdef __DREAMCAST__
    shz_sincos_t scZ = shz_sincosu16((u16)z);
    f32 sinZ = scZ.sin;
    f32 cosZ = scZ.cos;
#else
    ...
    if (y != 0) {
#ifdef __DREAMCAST__
        { shz_sincos_t sc = shz_sincosu16((u16)y); sin = sc.sin; cos = sc.cos; }
#else
        sin = Math_SinS(y);
        cos = Math_CosS(y);
#endif
    ...
    if (x != 0) {
#ifdef __DREAMCAST__
        { shz_sincos_t sc = shz_sincosu16((u16)x); sin = sc.sin; cos = sc.cos; }
#else
        sin = Math_SinS(x);
        cos = Math_CosS(x);
#endif
...
void SkinMatrix_SetRotateYXZ(MtxF* mf, s16 x, s16 y, s16 z) {
#ifdef __DREAMCAST__
    shz_mat4x4_init_rotation_yxz((shz_mat4x4_t*)mf,
                                 SHZ_S16_ANGLE(y),
                                 SHZ_S16_ANGLE(x),
                                 SHZ_S16_ANGLE(z));
#else
    f32 cos;
#ifdef __DREAMCAST__
    shz_sincos_t scY = shz_sincosu16((u16)y);
    f32 sinY = scY.sin;
    f32 cosY = scY.cos;
#else
    ...
    if (z != 0) {
#ifdef __DREAMCAST__
        { shz_sincos_t sc = shz_sincosu16((u16)z); sin = sc.sin; cos = sc.cos; }
#else
        sin = Math_SinS(z);
        cos = Math_CosS(z);
#endif
...
#ifdef __DREAMCAST__
    shz_mat4x4_init_translation((shz_mat4x4_t*)mf, x, y, z);
#else
    mf->yx = 0.0f;
    mf->zx = 0.0f;
    mf->wx = 0.0f;
...
void SkinMatrix_SetTranslateRotateZYXScale(MtxF* dest, f32 scaleX, f32 scaleY, f32 scaleZ, s16 rotX, s16 rotY, s16 rotZ,
    ...
    shz_xmtrx_rotate_zyx(SHZ_S16_ANGLE(rotZ), SHZ_S16_ANGLE(rotY), SHZ_S16_ANGLE(rotX));
    shz_xmtrx_scale(scaleX, scaleY, scaleZ);
...
#ifdef __DREAMCAST__
    shz_xmtrx_init_translation(translateX, translateY, translateZ);
    shz_xmtrx_rotate_yxz(SHZ_S16_ANGLE(rotY), SHZ_S16_ANGLE(rotX), SHZ_S16_ANGLE(rotZ));
    shz_xmtrx_scale(scaleX, scaleY, scaleZ);
    shz_xmtrx_store_4x4(dest);
#else
    MtxF mft1;
...
#ifdef __DREAMCAST__
    shz_mat4x4_init_rotation((shz_mat4x4_t*)mf, SHZ_S16_ANGLE(angle), axisX, axisY, axisZ);
#else
    f32 sinA;
    f32 cosA;
    f32 xx;
...
#ifdef __DREAMCAST__
    { shz_sincos_t sc = shz_sincosu16((u16)angle); sinA = sc.sin; cosA = sc.cos; }
#else
