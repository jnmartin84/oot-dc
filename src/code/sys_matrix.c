...
void Matrix_Translate(f32 x, f32 y, f32 z, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
#ifdef __DREAMCAST__
        shz_mat4x4_translate((shz_mat4x4_t*)cmf, x, y, z);
#else
        tx = cmf->xx;
        ty = cmf->xy;
...
void Matrix_Scale(f32 x, f32 y, f32 z, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
#ifdef __DREAMCAST__
        shz_mat4x4_scale((shz_mat4x4_t*)cmf, x, y, z);
#else
        cmf->xx *= x;
        cmf->yx *= x;
        cmf->zx *= x;
...
void Matrix_RotateX(f32 x, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
...
#ifdef __DREAMCAST__
            shz_mat4x4_rotate_x((shz_mat4x4_t*)cmf, x);
#else
            sin = sinf(x);
            cos = cosf(x);
        ...
        cmf = sCurrentMatrix;
#ifdef __DREAMCAST__
        shz_mat4x4_init_rotation_x((shz_mat4x4_t*)cmf, x);
#else
        if (x != 0) {
            sin = sinf(x);
            cos = cosf(x);
...
void Matrix_RotateY(f32 y, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
...
#ifdef __DREAMCAST__
            shz_mat4x4_rotate_y((shz_mat4x4_t*)cmf, y);
#else
            sin = sinf(y);
            cos = cosf(y);
        ...
        cmf = sCurrentMatrix;
#ifdef __DREAMCAST__
        shz_mat4x4_init_rotation_y((shz_mat4x4_t*)cmf, y);
#else
        if (y != 0) {
            sin = sinf(y);
            cos = cosf(y);
...
void Matrix_RotateZ(f32 z, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
...
#ifdef __DREAMCAST__
            shz_mat4x4_rotate_z((shz_mat4x4_t*)cmf, z);
#else
            sin = sinf(z);
            cos = cosf(z);
        ...
        cmf = sCurrentMatrix;
#ifdef __DREAMCAST__
        shz_mat4x4_init_rotation_z((shz_mat4x4_t*)cmf, z);
#else
        if (z != 0) {
            sin = sinf(z);
            cos = cosf(z);
...
void Matrix_RotateZYX(s16 x, s16 y, s16 z, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
#ifdef __DREAMCAST__
        shz_mat4x4_rotate_zyx((shz_mat4x4_t*)cmf,
                              SHZ_S16_ANGLE(z),
                              SHZ_S16_ANGLE(y),
                              SHZ_S16_ANGLE(x));
#else
...
void Matrix_TranslateRotateZYX(Vec3f* translation, Vec3s* rotation) {
    MtxF* cmf = sCurrentMatrix;
#ifdef __DREAMCAST__
    shz_xmtrx_load_4x4((const shz_mat4x4_t*)cmf);
    shz_xmtrx_translate(translation->x, translation->y, translation->z);
    shz_xmtrx_rotate_zyx(SHZ_S16_ANGLE(rotation->z), SHZ_S16_ANGLE(rotation->y), SHZ_S16_ANGLE(rotation->x));
    shz_xmtrx_store_4x4((shz_mat4x4_t*)cmf);
#else
...
void Matrix_SetTranslateRotateYXZ(f32 translateX, f32 translateY, f32 translateZ, Vec3s* rot) {
    MtxF* cmf = sCurrentMatrix;
#ifdef __DREAMCAST__
    shz_xmtrx_init_translation(translateX, translateY, translateZ);
    shz_xmtrx_rotate_yxz(SHZ_S16_ANGLE(rot->y), SHZ_S16_ANGLE(rot->x), SHZ_S16_ANGLE(rot->z));
    shz_xmtrx_store_4x4((shz_mat4x4_t*)cmf);
#else
...
Mtx* Matrix_MtxFToMtx(MtxF* src, Mtx* dest) {
    ...
    //ORIGINAL CODE
    /*  does not produce same result on n64 / PC 
    s32 temp;
...
#ifdef __DREAMCAST__
    *dest = shz_vec3_to(Vec3f, shz_mat4x4_transform_point3((shz_mat4x4_t*)sCurrentMatrix, shz_vec3_from(*src)));
#else
    MtxF* cmf = sCurrentMatrix;
...
#ifdef __DREAMCAST__
    shz_xmtrx_load_4x4((const float*)src);
    shz_xmtrx_store_4x4((float*)dest);
#else
    dest->xx = src->xx;
    dest->yx = src->yx;
    dest->zx = src->zx;
...
#ifdef __DREAMCAST__
    *dest = shz_vec3_to(Vec3f, shz_mat4x4_transform_point3((shz_mat4x4_t*)mf, shz_vec3_from(*src)));
#else
...
void Matrix_RotateAxis(f32 angle, Vec3f* axis, u8 mode) {
    ...
    if (mode == MTXMODE_APPLY) {
...
#ifdef __DREAMCAST__
            shz_mat4x4_rotate((shz_mat4x4_t*)cmf, angle, axis->x, axis->y, axis->z);
#else
            sin = sinf(angle);
            cos = cosf(angle);
        ...
        if (angle != 0) {
#ifdef __DREAMCAST__
            shz_mat4x4_init_rotation((shz_mat4x4_t*)cmf, angle, axis->x, axis->y, axis->z);
#else
            sin = sinf(angle);
            cos = cosf(angle);
...
#ifdef __DREAMCAST__
            shz_mat4x4_init_identity((shz_mat4x4_t*)cmf);
#else
            cmf->yx = 0.0f;
            cmf->zx = 0.0f;
            cmf->wx = 0.0f;
...
void Matrix_SetTranslateUniformScaleMtxF(MtxF* mf, f32 scale, f32 translateX, f32 translateY, f32 translateZ) {
    ...
    shz_xmtrx_set_translation(translateX, translateY, translateZ);
    shz_xmtrx_store_4x4((shz_mat4x4_t*)mf);
#else
    mf->xx = scale;
    mf->yx = 0.0f;
    mf->zx = 0.0f;
...
void Matrix_SetTranslateUniformScaleMtx2(Mtx* mtx, f32 scale, f32 translateX, f32 translateY, f32 translateZ) {
    Matrix_SetTranslateUniformScaleMtx(mtx, scale, translateX, translateY, translateZ);
...
void Matrix_SetTranslateScaleMtx1(Mtx* mtx, f32 scaleX, f32 scaleY, f32 scaleZ, f32 translateX, f32 translateY,
      ...
      MtxF mf;
    mf.xx = scaleX;  mf.xy = 0.0f;    mf.xz = 0.0f;    mf.xw = translateX;
    mf.yx = 0.0f;    mf.yy = scaleY;  mf.yz = 0.0f;    mf.yw = translateY;
    mf.zx = 0.0f;    mf.zy = 0.0f;    mf.zz = scaleZ;  mf.zw = translateZ;
    mf.wx = 0.0f;    mf.wy = 0.0f;    mf.wz = 0.0f;    mf.ww = 1.0f;
    guMtxF2L(mf.mf, mtx);
   ...
   /*  ORIGINAL CODE                               
    ...
    // Round-trip test  
    {
        ...
        if (count < 5) {
            f32 test[4][4];
            guMtxL2F(test, mtx);
            PRINTF("SetTranslateScaleMtx1: scaleX=%f -> [0][0]=%f [3][3]=%f (expect scaleX, 1.0)\n",
                   scaleX, test[0][0], test[3][3]);
            count++;
        }
    }
...
void Matrix_SetTranslateScaleMtx2(Mtx* mtx, f32 scaleX, f32 scaleY, f32 scaleZ, f32 translateX, f32 translateY,
    ...
    // Round-trip test  
    {
        ...
        if (count < 5) {
            f32 test[4][4];
            guMtxL2F(test, mtx);
            PRINTF("SetTranslateScaleMtx2: [0][0]=%f [1][1]=%f [2][2]=%f [3][3]=%f (expect %f, %f, %f, 1.0)\n",
                   test[0][0], test[1][1], test[2][2], test[3][3], scaleX, scaleY, scaleZ);
            count++;
        }
    }
