...
#define SHZ_S16_ANGLE(s) (((uint16_t)s) * (2.0f * SHZ_F_PI / (float)UINT16_MAX))
...
#ifdef __DREAMCAST__
#define DOTXYZ(vec1, vec2) (shz_dot6f((float)(vec1).x, (float)(vec1).y, (float)(vec1).z, (float)(vec2).x, (float)(vec2).y, (float)(vec2).z))
#else
